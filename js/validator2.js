...
function selectCountry(defaultSelectCountry){
    $('select[name="country2"]').children("option").attr("selected", false);
    $('select[name="country2"]').children('option[value="'+ defaultSelectCountry +'"]').attr("selected", true);
...
function changeSelect(dataId){
    if (boolChangeSelect){
        ...
        $('select[name="country2"]').children("option").attr("selected", false);
        $('select[name="country2"]').children('option[data-id="'+ dataId +'"]').attr("selected", true);
        $('select[name="country2"]').change();
...
$(".intlTelInput").each(function() {
    ...
    if ($(t).find("input[name='country2']").val() ) {

        if ($(t).find("input[name='country2']").val() != "RU"){

            libPhoneNumber ($(t).find(".phone"));
            country[countryEn] = $(t).find("input[name='country2']").val();
            $(t).find(".phone").intlTelInput("selectCountry", country[countryEn]);
            errorLanguage ($(t).find("input[name='country2']").val());
    ...
    if ($(t).find('select[name="country2"] option:selected').length > 0 ){

        dataIdDefault = $(t).find('select[name="country2"] option:selected').data('id');

        if ($(t).find('select[name="country2"] option:selected').val() != "RU") {

            libPhoneNumber ($(t).find(".phone"));
            country[countryEn] = $(t).find('select[name="country2"] option:selected').val();
            $(t).find(".phone").intlTelInput("selectCountry", country[countryEn]);
            errorLanguage ($(t).find("select[name='country2'] option:selected").val());
        ...
        $(t).find("select[name='country2']").change(function () {
            parentsChange = $(this).parents('.intlTelInput');
            if (parentsChange.find('select[name="country2"] option:selected').val() != "RU"){
                dataId = parentsChange.find('select[name="country2"] option:selected').data('id');
                ...
                errorLanguage (parentsChange.find("select[name='country2'] option:selected").val());
